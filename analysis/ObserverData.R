...
dat_pred = crossing(year = as.factor(1990:2018),
                    sfa = as.factor(c("6")),
                    size_mm = 0, #dummy value of length. Set to zero so it will not affect the predictions
                    sex.binom = 1 #dummy value of condition; will not affect predicted L50 values, but it is needed for building the model matrix.
                    #Temp.fall = 1, #dummy value of condition; will not affect predicted L50 values, but it is needed for building the model matrix.
                    #Sal.fall = 1, #dummy value of condition; will not affect predicted L50 values, but it is needed for building the model matrix.
                    #dens.year = 0
)


linear_model_matrix = model.matrix(l_mat_linear2$formula, dat_pred)

linear_coef = coef(l_mat_linear2)

L50_numerator = linear_model_matrix%*%linear_coef #matrix multiplication of 
L50_denominator = linear_coef[["size_mm"]]

dat_pred$L50_linear = -L50_numerator/L50_denominator

dat_pred$year<-as.integer(as.character(dat_pred$year))

ggplot(dat_pred,aes(x=year,y=L50_linear))+
  geom_point()+geom_line(size=1)+theme_bw()+
  ylab("L50 (mm)")+ xlab("Year")+
  #ylim(19.5, 22)+
...
data3$year<-as.factor(data3$year)
options(na.action = "na.fail")
l_mat_linear2 = glm(sex.binom ~ size_mm +year,#*sfa,#+Sal.all+Temp.all+dens.year,
                    family=binomial(link = "logit"),
                    data = data3)
